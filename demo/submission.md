...
**Atomic task unit (declared in writing, flagged to a mentor before judging):** one task = one real job posting or one outside submission processed end to end: live ATS fetch, dedupe, hard-filter verdict, fit score with caveats and evidence, sourced research brief, resume variant with named quality gates checked and per-gate results stored on the trace, LinkedIn-ready connection note (under 300 chars) and DM draft, delivery brief to the submitter's own Telegram or unique link, full trace row on the public ledger. Auto-rejected postings are logged but never counted as completed tasks. Failures stay visible on the ledger.
